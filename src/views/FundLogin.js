...
export default function SignIn() {
  ...
  const loginContinue = (e) => {
    e.preventDefault();
    setIsLoading(true);
    axios.post(`${process.env.REACT_APP_HOST}/fundraiser/checkEmail`, {
      email: email,
    }).then(res => {
  ...
  const loginFormSubmit = (e) => {
    e.preventDefault()
    setIsLoading(true);
    axios.post(`${process.env.REACT_APP_HOST}/fundraiser/login`, {
      email: email,
      password: password
    }).then(res => {
      setIsLoading(false);
      localStorage.setItem("fundraiser", res.data.token);
